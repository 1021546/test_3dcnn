...
all_mfec=np.empty(shape=[0,99,40])
...
for i in range(1,7):
	...
	speaker_mfec = np.reshape(speaker_mfec, (30, 99, 40))
	# print(speaker_mfec)
...
all_mfec = np.reshape(all_mfec, (6, 30, 99, 40))
...
if True:
	...
	one_file = np.reshape(one_file, (1, 99, 40))
	...
	x_test=np.empty(shape=[0, 99, 40])
	x_test = np.repeat(one_file,30,axis=0)

	print(x_test)
	print(x_test.shape) # (30, 99, 40)
	print(type(x_test))
	input()

	x_test = np.reshape(x_test, (1, 30, 99, 40))
...
# one speaker , one file duplicate
if False:
	x_test=np.empty(shape=[0,40])
	x_test = np.repeat(all_mfec[0][0],30,axis=0)

	print(x_test)
	print(x_test.shape) # (2970, 40)
	print(type(x_test))
	input()
...
if False:
	...
	for i in range(0,30):
			x_test=np.vstack((x_test,all_mfec[0][0]))
...
if False:
	...
	for n in range(0,1):
		for m in range(0,30):
			print(all_mfec[n][m].shape)
			print(x_test.shape)
			# input()
			x_test=np.vstack((x_test,all_mfec[n][m]))
...
if False:
	...
	for i in range(1,7):
		speaker_mfec=np.empty(shape=[0,40])
		for j in range(0,6):
			file_path = './wav/'+str(i)+'/'+str(j)+'_5.wav'
			file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),file_path)
			fs, signal = wav.read(file_name)
			print(type(signal))
			print(signal.shape)
			...
			# input()
			speaker_mfec=np.vstack((speaker_mfec,logenergy[0:99]))
		speaker_mfec = np.reshape(speaker_mfec, (6, 99, 40))
		# print(speaker_mfec)
		# print(speaker_mfec.shape)
		# print(type(speaker_mfec))
		all_mfec=np.vstack((all_mfec,speaker_mfec))
	...
	all_mfec = np.reshape(all_mfec, (6, 6, 99, 40))
...
# Define model
model = Sequential()
model.add(Reshape((30,99,40,1) , input_shape=(30,99,40)))
...
model.summary()
# plot_model(model, show_shapes=True, to_file='model_prelu.png')
...
def plot_history(history, result_dir):
	...
	plt.savefig(os.path.join(result_dir, 'model_accuracy_prelu.png'))
	plt.close()
	...
	plt.savefig(os.path.join(result_dir, 'model_loss_prelu.png'))
	plt.close()

# plot_history(history,os.path.dirname(os.path.abspath(__file__)))

# model.save('my_model_prelu.h5')
# del model


# Learn
# import numpy as np
# y_test=np.zeros((3,3,3))
# # [[[ 0.  0.  0.]
# #   [ 0.  0.  0.]
# #   [ 0.  0.  0.]]
