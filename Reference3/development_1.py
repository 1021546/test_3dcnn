...
for i in range(1,31):
	for j in range(1,51):
		...
		for k in range(0,20):
			...
			one_train_mfec=np.vstack((one_train_mfec,logenergy[0:99]))
			...
			# 	temp_j=j
			# 	temp_k=k
		one_train_mfec = np.reshape(one_train_mfec, (20, 99, 40))
...
# input()
all_speaker_mfec = np.reshape(one_speaker_mfec, (1500, 20, 99, 40))
...
'''
# np.save('./development_data/x_train.npy', x_train)
x_train = np.load('./development_data/x_train.npy')

# y_train=np.zeros(1500, dtype=np.int)
# for i in range(0,1500):
# 	label=i//50
# 	y_train[i]=label

# np.save('./development_data/y_train.npy', y_train)
y_train = np.load('./development_data/y_train.npy')
...
for i in range(1,31):
	for j in range(1,51):
		...
		for k in range(0,20):
			file_path = './development_wav/'+str(i)+'/test'+str(j)+'/'+str(k)+'.wav'
			file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)), file_path)
			fs, signal = wav.read(file_name)
			...
			one_test_mfec=np.vstack((one_test_mfec,logenergy[0:99]))
			...
			# 	temp_j=j
			# 	temp_k=k
		one_test_mfec = np.reshape(one_test_mfec, (20, 99, 40))
...
'''
# np.save('./development_data/x_test.npy', x_test)
x_test = np.load('./development_data/x_test.npy')

# y_test=np.zeros(1500, dtype=np.int)
# for i in range(0,1500):
# 	label=i//50
# 	y_test[i]=label


# np.save('./development_data/y_test.npy', y_test)
y_test = np.load('./development_data/y_test.npy')
...
# Define model
# model = Sequential()
# model.add(Reshape((20,99,40,1) , input_shape=(20,99,40)))
...
# model.add(BatchNormalization()) # BatchNormalization() can before activate function(more), also can after activate function(less)
model.add(PReLU(alpha_initializer='zeros', alpha_regularizer=None, alpha_constraint=None, shared_axes=None))
...
model.summary()
# plot_model(model, show_shapes=True, to_file='./development_data/development_model_1.png')
...
def plot_history(history, result_dir):
	...
	plt.show()
	# plt.savefig(os.path.join(result_dir, './development_data/development_accuracy.png'))
	plt.close()
	...
	plt.show()
	# plt.savefig(os.path.join(result_dir, './development_data/development_loss.png'))
	plt.close()

plot_history(history,os.path.dirname(os.path.abspath(__file__)))

model.save('./development_data/my_development_model_1.h5')
del model
