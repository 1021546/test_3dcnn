...
for i in range(1,5):
	for j in range(1,2):
		...
		for k in range(0,20):
			file_path = './enrollment_wav_2/'+str(i)+'/'+str(j)+'/'+str(k)+'.wav'
...
# input()
all_speaker_mfec = np.reshape(one_speaker_mfec, (4, 20, 99, 40))
...
# speaker_model_representation=np.empty(shape=[0,128])
# m=0
# for n in range(4):
# 	temp=np.mean(layer_output[m:m+21,:], axis=0)
